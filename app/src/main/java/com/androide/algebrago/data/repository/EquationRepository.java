package com.androide.algebrago.data.repository;
...
import com.androide.algebrago.centro.database.AppDatabase;
...
public class EquationRepository {
    ...
    private final com.androide.algebrago.data.local.dao.HistoryDao historyDao; // 🟢 NUEVA LÍNEA
    ...
    //puente traductor
    private List<com.androide.algebrago.domain.models.Term> stringToTerms(String side) {
        List<com.androide.algebrago.domain.models.Term> terms = new ArrayList<>();
        ...
        for (int i = 0; i < side.length(); i++) {
            ...
            if (c == '?' || c == '_') {
                ...
                terms.add(new com.androide.algebrago.domain.models.Term("t" + (termIdCounter++), com.androide.algebrago.domain.models.Term.TermType.BLANK, "?"));
            }
            // Si es un operador
            else if (c == '+' || c == '-' || c == '*' || c == '/' || c == '=') {
                flushBuffer(terms, buffer, termIdCounter++);
                terms.add(new com.androide.algebrago.domain.models.Term("t" + (termIdCounter++), com.androide.algebrago.domain.models.Term.TermType.OPERATOR, String.valueOf(c)));
            }
            // Si es un paréntesis
            else if (c == '(' || c == ')') {
                flushBuffer(terms, buffer, termIdCounter++);
                terms.add(new com.androide.algebrago.domain.models.Term("t" + (termIdCounter++), com.androide.algebrago.domain.models.Term.TermType.PARENTHESIS, String.valueOf(c)));
            }
     ...
     */
    private void flushBuffer(List<com.androide.algebrago.domain.models.Term> terms, StringBuilder buffer, int idCounter) {
        if (buffer.length() > 0) {
            String val = buffer.toString();
            // Si contiene algún número, lo tratamos como Constante. Si son letras, Variable.
            com.androide.algebrago.domain.models.Term.TermType type = val.matches(".*\\d.*") ?
                    com.androide.algebrago.domain.models.Term.TermType.CONSTANT :
                    com.androide.algebrago.domain.models.Term.TermType.VARIABLE;

            terms.add(new com.androide.algebrago.domain.models.Term("t" + idCounter, type, val));
     ...
     */
    public void insertHistory(com.androide.algebrago.data.local.entity.EquationHistoryEntity historyRecord) {
        AppDatabase.IO_EXECUTOR.execute(() ->
